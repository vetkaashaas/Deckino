...
public static class CardGeometryService
{
    ...
    public static Bitmap CreatePerspectivePreview(Bitmap image, IReadOnlyList<NormalizedPoint> points)
    {
        var validation = Validate(points);
        if (!validation.IsValid) throw new ArgumentException(validation.Message, nameof(points));

        using var source = image.Clone(new Rectangle(0, 0, image.Width, image.Height), PixelFormat.Format32bppArgb);
        var sourceStride = source.Width * 4;
        var sourcePixels = CopyPixels(source, sourceStride);

        var pixelCorners = points.Select(point => new NormalizedPoint(
            point.X * (source.Width - 1),
            point.Y * (source.Height - 1))).ToArray();
        ...
        for (var y = 0; y < PreviewHeight; y++)
            for (var x = 0; x < PreviewWidth; x++)
            {
                ...
                SampleBilinear(sourcePixels, source.Width, source.Height, sourceStride,
                    sourceX, sourceY, output, y * outputStride + x * 4);
            }
        var preview = new Bitmap(PreviewWidth, PreviewHeight, PixelFormat.Format32bppArgb);
        var bits = preview.LockBits(new Rectangle(0, 0, preview.Width, preview.Height),
            ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
        try
        {
            for (var row = 0; row < PreviewHeight; row++)
                Marshal.Copy(output, row * outputStride, bits.Scan0 + row * bits.Stride, outputStride);
        }
        finally
        {
            preview.UnlockBits(bits);
        }
        return preview;
    }

    private static byte[] CopyPixels(Bitmap bitmap, int destinationStride)
    {
        var output = new byte[destinationStride * bitmap.Height];
        var bits = bitmap.LockBits(new Rectangle(0, 0, bitmap.Width, bitmap.Height),
            ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
        try
        {
            for (var row = 0; row < bitmap.Height; row++)
                Marshal.Copy(bits.Scan0 + row * bits.Stride, output, row * destinationStride, destinationStride);
        }
        finally
        {
