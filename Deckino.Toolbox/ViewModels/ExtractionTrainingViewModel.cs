...
public partial class ExtractionTrainingViewModel : WorkspaceViewModel, IRefreshableWorkspace
{
    ...
    [ObservableProperty] public partial ImageSource? DiagnosticOverlayPreview { get; private set; }
    [ObservableProperty] public partial ImageSource? RectifiedPreview { get; private set; }
    [ObservableProperty] public partial ImageSource? RecognitionCropPreview { get; private set; }
    ...
    [RelayCommand]
    private async Task PickDiagnosticImageAsync()
    {
        var selectedPath = await _desktop.PickFileAsync(
            "Choose an owned camera photograph",
            [".jpg", ".jpeg", ".png"]);
        if (selectedPath is not null) DiagnosticImagePath = selectedPath;
    }
    ...
    private void OpenLogFolder()
    {
        ...
        _desktop.OpenFolder(_paths.LogsRoot);
    }
    ...
    private async Task CopySelectedLogAsync()
    {
        if (!string.IsNullOrWhiteSpace(SelectedLogLine))
            await _desktop.CopyTextAsync(SelectedLogLine);
    }
    ...
    private async Task CopyAllLogsAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(text)) await _desktop.CopyTextAsync(text);
    }
    ...
    private async Task ClearAllLogsAsync()
    {
        var confirmed = await _desktop.ConfirmAsync(
            "Clear activity log",
            "Clear the visible activity log and delete the current Deckino Toolbox log file?",
            "Clear log");
        if (!confirmed) return;
    ...
    internal static ImageSource? LoadUnlockedBitmap(string path)
    {
        if (string.IsNullOrWhiteSpace(path) || !File.Exists(path)) return null;
        return Application.Current is null ? null : ImageSourceFactory.FromFileUnlocked(path);
    }
