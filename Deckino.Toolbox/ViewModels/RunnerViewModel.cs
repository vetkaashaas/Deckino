...
public partial class RunnerViewModel : WorkspaceViewModel
{
    ...
    private async Task InstallPackagesAsync()
    {
        ...
        {
            allowPythonInstall = await _desktop.ConfirmAsync(
    ...
    private async Task RunProductionAsync()
    {
        await RunGuardedAsync("Running the full identity workflow…", async cancellationToken =>
        {
            ...
            var snapshot = await _identityProduction.RunAsync(
                profile,
                AppendLog,
                value => _desktop.BeginInvoke(() => ApplyProductionSnapshot(value)),
                cancellationToken);
    ...
    [RelayCommand]
    private async Task PickCameraFolderAsync()
    {
        CameraCaptureFolder = await _desktop.PickFolderAsync("Select labeled camera captures")
            ?? CameraCaptureFolder;
    }
    ...
    [RelayCommand]
    private async Task PickRecognitionImageAsync()
    {
        RecognitionImage = await _desktop.PickFileAsync(
            "Select a saved card image",
            [".jpg", ".jpeg", ".png", ".webp"])
            ?? RecognitionImage;
    }
    ...
    {
        Directory.CreateDirectory(_paths.TrainingRoot);
        _desktop.OpenFolder(_paths.TrainingRoot);
    }
    ...
    private async Task ClearAllLogsAsync()
    {
        ...
        catch (Exception error)
        {
            ...
            await _desktop.ShowMessageAsync("Clear activity log failed", Status);
        }
    }
