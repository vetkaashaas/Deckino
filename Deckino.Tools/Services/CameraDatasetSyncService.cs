...
public sealed class CameraDatasetSyncService : ICameraDatasetSyncService
{
    ...
    public async Task TestConnectionAsync(DatasetSyncCredentials credentials, CancellationToken cancellationToken)
    {
        ...
        await remote.ListAsync("camera/v1/", cancellationToken);
    }
    ...
    public async Task<DatasetSyncResult> SyncNowAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _localFiles = EnumerateLocalFiles().Count;
            await using var remote = _remoteFactory.Create(credentials);
            var uploaded = await ProcessPendingCoreAsync(remote, includeDeferred: true, cancellationToken);
            var errors = new List<string>();
            var downloaded = 0;
            var deleted = 0;
            var conflicts = 0;

            var remoteObjects = await remote.ListAsync("camera/v1/", cancellationToken);
            ...
            var tombstones = await LoadTombstonesAsync(remote, remoteObjects, cancellationToken);
            RaiseStateChanged();
            ...
            foreach (var tombstone in tombstones.Values)
            {
                ...
                {
                    await remote.DeleteAsync(tombstone.ObjectKey, cancellationToken);
            ...
            foreach (var remoteEntry in remoteFiles)
            {
                ...
                try
                {
                    remoteMaterial = await MaterializeRemoteAsync(remote, remoteInfo, cancellationToken);
                    if (!File.Exists(localPath))
                    {
            ...
            uploaded += await ProcessPendingCoreAsync(remote, includeDeferred: true, cancellationToken);
