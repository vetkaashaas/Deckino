    ...
    return transforms.Compose(
        [
            transforms.RandomResizedCrop(
                IMAGE_SIZE, scale=(0.72, 1.0), ratio=(0.78, 1.28)
            ),
            ...
            transforms.RandomApply(
                [transforms.GaussianBlur(5, sigma=(0.1, 1.8))], p=0.3
            ),
            transforms.ToTensor(),
            RandomGlare(),
    ...
    if simulated_camera:
        operations.extend(
            [
                ...
                transforms.RandomApply(
                    [transforms.GaussianBlur(3, sigma=(0.1, 0.7))], p=0.25
                ),
            ]
        )
    operations.extend(
...
class CardDataset(Dataset[tuple[Tensor, int, str]]):
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, int, str]:
        ...
        with Image.open(self.root / record.image_path) as source:
            image = crop_card_art(source.convert("RGB"), record.input_kind)
        if self.deterministic:
            seed = record.augmentation_seed
            if seed is None:
                seed = int(
                    hashlib.sha256(record.printing_id.encode("utf-8")).hexdigest()[:15],
                    16,
                )
...
class ArcMarginProduct(nn.Module):
    ...
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        # Equality avoids allocating a separate integer one-hot tensor.
        class_indices = torch.arange(self.weight.shape[0], device=labels.device)
        one_hot = labels.unsqueeze(1).eq(class_indices).to(dtype=cosine.dtype)
...
def collect_predictions(
    ...
    for images, labels, _printing_ids in loader:
        ...
        values, indices = similarities.topk(top_count, dim=1)
        for row_index, actual in enumerate(labels.tolist()):
            best_score = float(values[row_index, 0].item())
            runner_up = (
                float(values[row_index, 1].item()) if top_count > 1 else -1.0
            )
            result.append(
                Prediction(
                    actual=actual,
                    predicted=int(indices[row_index, 0].item()),
                    top5_correct=actual in indices[row_index].tolist(),
...
def summarize_predictions(
    ...
    top5 = sum(item.top5_correct for item in predictions)
    accepted_predictions = [
        item
        for item in predictions
        if item.cosine_score >= score_threshold and item.margin >= margin_threshold
    ]
    accepted_correct = sum(
        item.actual == item.predicted for item in accepted_predictions
    )
    ...
    return EvaluationResult(
        samples=samples,
        top1=correct / samples if samples else 0.0,
        top5=top5 / samples if samples else 0.0,
        accepted_precision=accepted_correct / accepted if accepted else 0.0,
        coverage=accepted / samples if samples else 0.0,
        accepted=accepted,
...
def calibrate_thresholds(
    ...
    if not predictions:
        raise ValueError("Cannot calibrate thresholds without predictions")
    scores = np.asarray([item.cosine_score for item in predictions])
    margins = np.asarray([item.margin for item in predictions])
    correct = np.asarray([item.actual == item.predicted for item in predictions])
    candidates: list[tuple[bool, float, float, float, float]] = []
    for score_threshold in np.linspace(-0.2, 0.9, 111):
        score_mask = scores >= score_threshold
        for margin_threshold in np.linspace(0.0, 0.30, 61):
            accepted = score_mask & (margins >= margin_threshold)
            accepted_count = int(accepted.sum())
            precision = float(correct[accepted].mean()) if accepted_count else 0.0
            coverage = accepted_count / len(predictions)
            qualified = precision >= target_precision and coverage >= minimum_coverage
            candidates.append(
                (
                    qualified,
                    coverage,
    ...
    qualified_candidates = [item for item in candidates if item[0]]
    if qualified_candidates:
        ...
        selected = max(candidates, key=lambda item: (item[2], item[1], -item[3], -item[4]))
    qualified, coverage, precision, score_threshold, margin_threshold = selected
