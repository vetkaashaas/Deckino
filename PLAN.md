...
Real-photo-first recipe (architecture `mobilenetv3-small-spatial-v2`, training recipe 3; CLI 0.9.2):
...
- Recipe 3 keeps the 256px network and inference cost unchanged. Real samples target 75% positive / 25% negative; within each class, mix equal natural-photo and uniform capture-group sampling with replacement. Missing classes are explicit; enabled synthetic stays capped at 20%. Coordinate loss averages 50% mean and 50% worst-corner loss per positive image, logging both terms.
- Main training is followed automatically by 20 precision-finishing epochs from its best checkpoint, using unaugmented real photos only, reset optimizer/scaler, and fixed backbone/head rates 3e-6 / 3e-5. Preserve the best checkpoint across both phases and persist the phase transition, recipe/objective/sampling policy, and RNG/optimizer state. Older recipes remain preview-compatible but cannot resume into recipe 3.
...
- Recipe-3 exports require `calibration.json` alongside existing schema-v2 artifacts, including phase history, comparison evidence and validation/test review overlays. Local execution validation is deferred to the NVIDIA machine at the user's request; no accuracy improvement is claimed before that run.
