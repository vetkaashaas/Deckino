...
def transform_photo(image: Image.Image, corners: Sequence[dict[str, float]] | None,
    ...
    inverse /= inverse[2, 2]
    result = image.transform(image.size, Image.Transform.PERSPECTIVE, inverse.flatten()[:8],
...
def augment_photo(image: Image.Image, corners: Sequence[dict[str, float]] | None,
    ...
    if corners is not None and rng.random() < 0.8:
        width, height = image.size
        center = np.array([[1, 0, -(width - 1) / 2], [0, 1, -(height - 1) / 2], [0, 0, 1.]])
        for _ in range(8):
            angle = math.radians(rng.uniform(-25, 25) if rng.random() < 0.8 else rng.uniform(-180, 180))
